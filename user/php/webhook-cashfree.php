...
ini_set ('display_errors', 'on');
 ...
 chmod("cashfree-webhook.php",0644);
require '../config/functions.php';
$data = file_get_contents("php://input");
$result = json_decode($data, true);
$curl = curl_init();
  $orderStatus = $result["type"];
    $result = $result['data'];
  ...
  CURLOPT_URL => "https://api.cashfree.com/pg/orders/" . $orderDetails["order_id"],
  ...
  CURLOPT_HTTPHEADER => [
    ...
    "x-client-id: 207809d3a66e4627762d1d7def908702",
     "x-client-secret: d567403591de4b2bf863b4269350f717f6e1b804"
  ],
]);
 ...
 $result = json_decode($response, true);
    ...
    $sql = "INSERT INTO `payment_details` SET `account_id`=?,`payment_id`=?,`payment_amount`=?,`order_id`=?,`method`=?,`email`=?,`acquirer_data`=?,`user_id`=?";
      if(sqlInsert($sql,'ssssssss',$account_id,$payment_id,$payment_amount,$payment_order_id,$payment_method,$payment_email,$payment_acquirer_data,$payment_user_id)){
        $sql = "UPDATE `users` SET `secret_key`=?,`package`=?,`status`='ACTIVE' WHERE `order_id` =? AND `userId` =?";
        $activeSql = sqlUpdate($sql,'ssss',$secret_id,$packageId,$payment_order_id,$payment_user_id);
        if($activeSql){
            ...
            $checkDirectComm = "SELECT * FROM `reference` WHERE `User_id` = '".$payment_user_id."' AND `Reff_User_id` = '".$DirectId."' AND `Ref_amount` = '".$directValue."'";
            ...
            $directtatus = sqlSelector($statusCheck);
            if(($directtatus[0]['package'])>=0){
            $checkPassiveComm = "SELECT * FROM `reference` WHERE `User_id` = '".$payment_user_id."' AND `Reff_User_id` = '".$PassiveId."' AND `Ref_amount` = '".$passiveValue."'";
